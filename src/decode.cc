...
void Reconstruct(const FrameHeader &header, const FrameTag &tag, const std::array<Frame, 4> &refs, const std::array<bool, 4> &ref_frame_bias, BitstreamParser &ps, Frame &frame) {
  ...
  for (size_t r = 0; r < frame.vblock; ++r) {
    for (size_t c = 0; c < frame.hblock; ++c) {
      ...
      if (pre.is_inter_mb) InterPredict(r, c, tag, refs, ref_frame_bias, pre.ref_frame, interc, ps, frame);
