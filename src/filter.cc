...
int16_t LoopFilter::Adjust(bool use_outer_taps) {
  ...
  int16_t a =
      int16_t(Clamp128((use_outer_taps ? Clamp128(P1 - Q1) : 0) + 3 * (Q0 - P0)));
...
template <size_t C>
void FrameFilter(FrameHeader &header, uint8_t sharpness_level, std::vector<std::vector<MacroBlock<C>>> frame, size_t hblock, size_t vblock, bool is_key_frame, LoopFilter &filter) {
	for(size_t r = 0; r < vblock; r++) {
		for(size_t c = 0; c < hblock; c++) {
			...
			int16_t edge_limit_mb = (int16_t(loop_filter_level + 2) * 2) + int16_t(interior_limit);
			int16_t edge_limit_sb = (int16_t(loop_filter_level) * 2) + int16_t(interior_limit);
			...
			if(c > 0) {
				...
				for(size_t i = 0; i < C; i++) {
					...
					for(size_t j = 0; j < 4; j++) {
						...
						filter.MacroBlockFilter(hev_threshold, interior_limit, edge_limit_mb);
				...
				for(size_t i = 0;i < C;i++) {
					for(size_t j = 1; j < C; j++) {
						...
						for(size_t a = 0; a < 4; a++) {
							...
							filter.SubBlockFilter(hev_threshold, interior_limit, edge_limit_sb);
